## Доска Bamboo для большого монитора
Отображает состояние сборки проектов(указанных в `projectNames`, в файле
`src/Domain/consts.js`). Так же в доску встроен таймер СО2 - когда пора
проветрить комнату. Срабатывает каждые 45 минут и длится 5минут. 

Для запуска, достаточно собрать и запустить `python`
сервер в каталоге сборки:

    npm install
    npm run build
    python3 -m http.server 8000

После этого достаточно 1 раз открыть страницу `localhost:8000` в
браузере(chrome), поддерживающем Service-Workers. После этого можно сервер 
погасить, страница в вашем браузере будет доступна по прежнему. Потому что
ServiceWorkers(гуглите, или почитайте 
[Offline-First Considerations](#Offline-First Considerations)).

## Описание от react-create-app
